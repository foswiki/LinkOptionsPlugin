...
# =========================
package Foswiki::Plugins::LinkOptionsPlugin
...
@preDefOptions = ( 'newwin', 'name', 'title', 'class', 'id', 'skin' );
@winOptions = (
    'directories', 'location', 'menubar',  'resizable',
...
sub handleLinkOptions {
    ...
    if ( $extraOpts{'skin'} ) {
        if ( $html =~ m/^<a [^>]*href="[^"]*\?[^"]*skin=.+/ ) {
            $html =~
s/^<a ([^>]*href="[^"]*\?[^"]*skin=)[^&"]*(.+)/<a $1$extraOpts{'skin'}$2/
              ;    #"
        }
        else {
            if ( $html =~ m/^<a [^>]*href="[^"]*\?.+/ ) {
                $html =~
s/^<a ([^>]*href="[^"]*\?)(.+)/<a $1skin=$extraOpts{'skin'}&$2/
                  ;    #"
            }
            else {
                $html =~
s/^<a ([^>]*href="[^"]*)"(.+)/<a $1?skin=$extraOpts{'skin'}"$2/
                  ;    #"
            }
        }
    }
    ...
    if ( $extraOpts{'title'} ) {
        if ( $html =~ m/^<a [^>]*title=.+/ ) {
            $html =~
s/^<a ([^>]*)title="[^"]*"([^>]*)>(.+)/<a $1$2 title="$extraOpts{'title'}">$3/
              ;                                                          #"
        }
        else {
    ...
    if ( $extraOpts{'class'} ) {
        if ( $html =~ m/^<a [^>]*title=.+/ ) {
            $html =~
s/^<a ([^>]*)class="[^"]*"([^>]*)>(.+)/<a $1$2 class="$extraOpts{'class'}">$3/
              ;                                                          #"
        }
        else {
    ...
    if ( $extraOpts{'id'} ) {
        if ( $html =~ m/^<a [^>]*id=.+/ ) {
            $html =~
s/^<a ([^>]*)id="[^"]*"([^>]*)>(.+)/<a $1$2 id="$extraOpts{'id'}">$3/
              ;                                                          #"
        }
        else {
